...
/*
 * USDZ model loader by Syoyo Fujita
 */
...
namespace vkUSDZ
{
	...
	void Texture::fromUSDZImage(tinyusdz::tydra::TextureImage &usdzimage, const std::vector<uint8_t> &imagedata, TextureSampler textureSampler, vks::VulkanDevice *device, VkQueue copyQueue)
	{
		...
		if (usdzimage.channels == 3) {
			...
			const unsigned char* rgb = reinterpret_cast<const unsigned char *>(&imagedata[0]);
			...
			buffer = reinterpret_cast<unsigned char *>(const_cast<uint8_t *>(&imagedata[0]));
		...
		// Generate the mip chain (USDZ uses jpg and png, so we need to create this manually)
	...
	{
		if (animations.empty()) {
			std::cout << ".USDZ does not contain animation." << std::endl;
			return;
		}
