...
public:
	...
	void recordCommandBuffer(bool is_usdz)
	{
	...
	void loadAssets(bool use_usdz = true)
	{
		...
		for (size_t i = 0; i < args.size(); i++) {
			if (use_usdz) {
        if ((std::string(args[i]).find(".usdz") != std::string::npos) || (std::string(args[i]).find(".usda") != std::string::npos) || (std::string(args[i]).find(".usdc") != std::string::npos)) {
	...
	*/
	...
	virtual void render(bool is_usdz)
	{
		if (!prepared) {
			return;
		}
